...
export const IconButton = forwardRef<HTMLButtonElement, IconButtonProps>(
  function IconButton({ label, icon, style, tooltip = true, ...props }, ref) {
    const button = (
        <Button
          {...props}
          ref={ref}
...
export interface SelectMenuProps {
  trigger: ReactElement<Pick<HTMLAttributes<HTMLElement>, "onPointerDown" | "onClick">>;
  label?: string;
  options: MenuOption[];
  value: string;
...
export const Select = forwardRef<HTMLButtonElement, SelectProps>(function Select({
  options,
  label,
  showLabel = false, variant = "compact", wrapperClassName,
  placeholder = "Choose…",
  className,
  style,
  ...
  return (
    ...
    <RadixSelect.Root value={value} defaultValue={defaultValue} onValueChange={onValueChange} disabled={disabled} name={name} required={required}>
      <RadixSelect.Trigger
        ...
        className={cx(variant === "field" ? field().root : button({ variant: "tonal" }), className)}
        data-disabled={disabled || undefined}
        style={{ display: "flex", gap: 12, color: "var(--il-ink)", ...(variant === "field" ? { width: "100%", padding: "11px 14px", cursor: disabled ? "not-allowed" : "pointer" } : {}), justifyContent: "space-between", ...style }}
      >
...
> (function TabsList({ enclosed = false, style, children, ...props }, ref) {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const next = active ? { x: active.offsetLeft, y: active.offsetTop, width: active.offsetWidth, height: active.offsetHeight } : null;
      setIndicator(previous => previous?.x === next?.x && previous?.y === next?.y && previous?.width === next?.width && previous?.height === next?.height ? previous : next);
    };
    ...
    changes.observe(list, { subtree: true, childList: true, attributes: true, attributeFilter: ["data-state"] });
    observe();
    return () => { resize.disconnect(); changes.disconnect(); };
  }, []);
  return <RadixTabs.List ref={attachRef} {...props} data-nc-tabs="" data-indicator={indicator ? "ready" : undefined} style={{
    ...
    {indicator && <span aria-hidden="true" data-nc-tab-indicator="" style={{
...
>(function TabsContent({ style, ...props }, ref) {
  return <RadixTabs.Content ref={ref} {...props} data-nc-tab-panel="" style={{ paddingTop: 16, ...style }} />;
});

...
`onSend` receives `{ text, model, reasoning, replyFormat, goal?, instruction? }`. Saved context is included when nonempty. The package never clears a draft automatically; clear after the application accepts submission. Enter sends; Shift+Enter adds a line; IME composition does not send. Writing grows from one to eight lines, then scrolls. Controls retain their positions as the writing area grows.
...
Supply `goal/onGoalChange` and `instruction/onInstructionChange` for saved context pills inside the composer. Their Add entries open an anchored editor; Save trims the text, Cancel or Escape discards the edit. Hover or keyboard focus reveals the saved text; clicking the pill edits it, and its remove button clears it. Context persists until the host changes it, including after sending. At narrow widths the pills move to a row above the controls inside the composer. Without a change handler, the corresponding Add entry continues calling `onAdd`. `goalIcon` overrides the default Lucide Goal consistently in the menu and pill. The preview uses this library default. The approved behavior is recorded in [the composer context contract](INTERACTION-PATTERNS.md#composer-context-pills).
...
`Surface` and the host `surface` recipe share the task graph's five-level material system. With the default `texture="auto"`, `ground` and `chrome` use 28px graph paper at 7% line intensity. `elevated`, `elevated-2`, and `top` use glass with 64% fill, 20px blur, and the corresponding `sys-elev-1/2/3` shadows. Dense artifact editors can use `texture="plain"` on a raised layer for an opaque reading plane inside graph chrome. The theme supplies each layer's color; disabling glass makes glass surfaces opaque and removes their blur.
...
## Approved spacing relationships

Use `FieldGroup`, `SectionStack`, `Section`, `InlineCluster`, `ActionRow`, `ResponsiveGrid`, `Inset`, and `Stack` from `@nova-caelum/ui` for composition. Include the package stylesheet and use `CaelosProvider` as usual. Default requires no configuration: 21px fields, 27px sections, 15px intro-to-content and 21px inset. Wrap a region in `<SpacingDensity value="compact">` or `value="comfortable"` for an explicit alternative; `value="default"` resets a nested region to the standard. Hosts own preference state/persistence.

These layouts forward refs and native element props, own sibling gaps/padding, and leave field interiors unchanged. `FieldGroup` is a div, not a semantic fieldset. `Section` supplies an associated heading (`level` 2–4) and optional description. Use one inset owner per surface. `spacing({ kind, density })` is also available from `@nova-caelum/ui/recipes`; density defaults to `default`. The preset and built stylesheet include every density.

See [spacing reference](../../docs/design/SPACING-RELATIONSHIPS.md) and the **Spacing** tab in the React 19 Foundry. Atlas 3 is a review harness using the same exports. Existing product layouts require deliberate adoption; these additions do not globally restyle margins.

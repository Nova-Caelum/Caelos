...
**Principle: states and information evolve from awareness to understanding, and to action when warranted, within the same flow.**

Stated by the user, 2026-09-19: "the design language principle is that states and information evolves from awareness -> understanding. ie: there should be a small indicator which easily flows into a detail through expansion, and hopefully action when warranted within the same flow."

A small indicator (awareness) expands in place into detail (understanding) and, when warranted, into an action — without leaving the flow or opening a disconnected surface. *Derived, pending user review:* at any one level of a surface there should be one expression of a given state, not several parallel ones — e.g. an avatar badge is the awareness-level signal; an expanded card's status block with reason and action is the understanding/action level; a second status icon inside that expanded card would be a redundant awareness-level signal competing with it.

Related to [Progressive Selector](#progressive-selector): Progressive Selector governs compact, reversible **settings** (the control stays stable, detail reveals on demand); this principle governs **state and information** disclosure generally. They are siblings, not the same rule.

*Acceptance checks — derived, pending user review:*

1. Each state has exactly one indicator at rest per surface level; expanding it does not leave a second, parallel indicator for the same state visible at that level.
2. Expansion reveals detail (and action, when warranted) in place, through the same control/flow — not a separate disconnected surface or navigation.
3. Where action is available, it is reachable from the expanded detail without exiting the flow that started at the awareness indicator.

...
## Approved input diffusion — September 19, 2026 (React 19)

Promoted the approved Foundry trial into the shared input recipe. Hover applies only to enabled, unfocused fields; actual/forced focus retains full diffusion independent of generated rule ordering. The same recipe serves Input and TextArea.

- Shared Panda generation, TypeScript, ESM/declarations and stylesheet build passed.
- Refreshed the pinned React 19 Foundry snapshot and removed its temporary CSS override; isolated gallery TypeScript/Vite build passed.
- Browser computed opacity: focused with pointer inside 1; focused with pointer outside 1; unfocused hover 0.22; disabled field 0. Final focused render inspected.
- Local package and staging update only; no React 18 baseline change or remote publication.

## Composer Add menu fallback — September 19, 2026 (React 19)

- Reproduced a 53.825px gap below the composer: Radix reused the above-composer offset when flipping the Add menu.
- The shared surface docking hook now measures the composer shell and chooses the side and offset together. Add retains its 2px top cushion and uses a 0.5px bottom cushion, following scrolling while open.
- Shared Panda generation, TypeScript, ESM/declarations and stylesheet build passed; refreshed isolated Foundry TypeScript/Vite build passed.
- Final browser check after scrolling: bottom placement measured 0.225px (subpixel/device rounding); top placement measured 1.775px. Render inspected. Add menu receives focus, Escape returns focus to Add, and Goal opens its focused editor; Cancel dismisses it.
- Refreshed local Foundry snapshot: `nova-caelum-ui-a4b8b2d461faa390.tgz`. Targeted browser checks only; the full historical browser suite was not rerun. No live deployment.
